...
func (in *IronicStatus) DeepCopyInto(out *IronicStatus) {
	*out = *in
	...
	if in.InspectorServiceIDs != nil {
		in, out := &in.InspectorServiceIDs, &out.InspectorServiceIDs
		*out = make(map[string]string, len(*in))
		for key, val := range *in {
			(*out)[key] = val
		}
	}
	if in.InspectorAPIEndpoints != nil {
		in, out := &in.InspectorAPIEndpoints, &out.InspectorAPIEndpoints
		*out = make(map[string]map[string]string, len(*in))
		for key, val := range *in {
			var outVal map[string]string
			if val == nil {
