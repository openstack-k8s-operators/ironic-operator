...
- admissionReviewVersions:
  ...
  clientConfig:
    caBundle: ${CA_BUNDLE}
    url: https://${CRC_IP}:${WEBHOOK_PORT}/validate-ironic-openstack-org-v1beta1-ironic
  failurePolicy: Fail
...
- admissionReviewVersions:
  ...
  clientConfig:
    caBundle: ${CA_BUNDLE}
    url: https://${CRC_IP}:${WEBHOOK_PORT}/mutate-ironic-openstack-org-v1beta1-ironic
  failurePolicy: Fail
  matchPolicy: Equivalent
  name: mironic.kb.io
  objectSelector: {}
...
if [ -n "${CSV_NAME}" ]; then
    ...
    if CR_NAME=$(oc get openstack -n openstack-operators -o name); then
        ...
        To restore it, use:
        oc patch ${CR_NAME} -n openstack-operators --type=merge -p '{\"spec\": {\"operatorOverrides\": [{\"name\": \"ironic\", \"replicas\": 1}]}}'\n"

        oc patch ${CR_NAME} -n openstack-operators --type=merge -p '{"spec": {"operatorOverrides": [{"name": "ironic", "replicas": 0}]}}'
    fi
fi
