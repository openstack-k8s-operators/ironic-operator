...
var _ = Describe("IronicInspector controller", func() {
	...
	When("An ApplicationCredential is created for IronicInspector", func() {
		...
		BeforeEach(func() {
			...
			// Create AC secret for inspector - the controller reads this directly
			acSecretName = "ac-ironic-inspector-secret"
			acSecret := &corev1.Secret{
				...
				Data: map[string][]byte{
					"AC_ID":     []byte("test-inspector-ac-id"),
					"AC_SECRET": []byte("test-inspector-ac-secret"),
				},
			}
