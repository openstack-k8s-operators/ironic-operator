...
var _ = Describe("IronicInspector controller", func() {
	...
	When("A IronicInspector is created with HttpdCustomization.CustomConfigSecret", func() {
		BeforeEach(func() {
			customServiceConfigSecretName := types.NamespacedName{Name: "foo", Namespace: ironicNames.Namespace}
...
CustomKeystonePublicURL "{{ .KeystonePublicURL }}"`)
			...
			spec := GetDefaultIronicInspectorSpec()
			spec["rpcTransport"] = "oslo"
			spec["keystoneEndpoints"] = map[string]interface{}{
		...
		It("it renders the custom template and adds it to the ironic-inspector-config-data secret", func() {
			scrt := th.GetSecret(ironicNames.InspectorConfigSecretName)
